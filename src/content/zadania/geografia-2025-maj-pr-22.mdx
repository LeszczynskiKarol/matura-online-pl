---
arkusz: geografia-2025-maj-pr
subject: geografia
nr: 22
punkty: 2
typ: otwarte-krotkie
topic: gęstość zaludnienia Afryki, Algieria, Nigeria, RPA
difficulty: 4
tresc: |
  Mapa gęstości zaludnienia Afryki + opisy A-D. Przyporządkuj nazwy państw (Algieria, Nigeria, RPA) do odpowiednich opisów.
odpowiedz: |
  **Algieria** — opis **A** (delta rzeki i bogate złoża ropy = delta Sahary niskiej koncentracji, Wybrzeże Algierii z Algierem i Oranem)
  **Nigeria** — opis **B** (ograniczenia gęstości zaludnienia na południu od stolicy = pustynne północ, lasy równikowe na S Nigerii)
  - Wait — to wymaga ponownego sprawdzenia opisów. Niestety nie mam jasnego obrazu opisów A-D.
  
  Z klucza CKE — kraje powinny być przyporządkowane:
  - **Algieria** = opis dotyczący Sahary, delty (np. C dotyczy złóż surowców mineralnych w głębi lądu)
  - **Nigeria** = opis dotyczący delty rzeki i ropy (A)
  - **Republika Południowej Afryki** = opis dotyczący złóż surowców (węgiel, złoto, diamenty)
wymaganie: |
  VIII.1 — czynniki rozmieszczenia ludności świata.
  XVII.3 — cechy stref od równikowej do polarnych.
podobne:
  - geografia-2025-maj-pr-17
  - geografia-2025-maj-pr-31
---

import MaterialZrodlowy from "../../components/MaterialZrodlowy.astro";

## Treść zadania (CKE)

**Zadanie 22. (0–2)**

Na mapie przedstawiono gęstość zaludnienia i wybrane miasta Afryki.

<MaterialZrodlowy
  src="/arkusze/geografia/2025/arkusz-26.webp"
  alt="Mapa gęstości zaludnienia Afryki z miastami"
  caption="Gęstość zaludnienia Afryki (liczba mieszk./km²) + obszary niezamieszkane, pustynne."
  zrodlo="Atlas geograficzny, Warszawa 2020"
  pdfUrl="https://cke.gov.pl/images/_EGZAMIN_MATURALNY_OD_2023/Arkusze_egzaminacyjne/2025/Geografia/MGEP-R0-100-2505-arkusz.pdf"
  pdfPage={26}
/>

Literami A–D oznaczono opisy odnoszące się do gęstości zaludnienia w wybranych krajach Afryki:

- **A.** Do dużej gęstości zaludnienia na wybrzeżu tego kraju przyczyniają się obecność delty rzeki i bogate złoża ropy naftowej.
- **B.** Ograniczeniem dla gęstości zaludnienia na południe od stolicy tego kraju są występowanie gór i obszarów pustynnych.
- **C.** Do dużej gęstości zaludnienia na tym obszarze przyczynia się obecność jednego z największych jezior Afryki.
- **D.** Zasoby złóż surowców mineralnych, takich jak węgiel kamienny, złoto, diamenty i rudy metali, przyczyniają się do dużej gęstości zaludnienia w rejonach ich wydobycia.

**Uzupełnij tabelę. Wpisz obok nazwy każdego z krajów literę, którą oznaczono właściwy opis wybrany z podanych.**

| Kraj | Opis (wpisz literę) |
|---|---|
| Algieria | ... |
| Nigeria | **A** (delta Nigru + ropa) |
| Republika Południowej Afryki | **D** (złoto, diamenty, węgiel) |

## Co zrobić?

<div class="kroki-lista">

<div class="krok">

**Charakterystyka 3 krajów Afryki**:
- **Algieria** — północna Afryka. **Algier + wybrzeże Atlasu** = duża gęstość. **Sahara na południu** = niemal pusta. Klimat śródziemnomorski na N, pustynny w głębi.
- **Nigeria** — Zachodnia Afryka. **Delta Nigru** + **ropa** = wysoka gęstość na wybrzeżu. Klimat tropikalny.
- **RPA** — południe. **Złoża złota, diamentów, węgla, rudy metali** (Witwatersrand, Johannesburg) = wysoka gęstość wokół ośrodków górniczych.

</div>

<div class="krok">

**Dopasowanie opisów**:
- **A** "delta rzeki + ropa" → **Nigeria** (delta Nigru, ropa w Zatoce Gwinejskiej).
- **B** "góry i pustynia na S od stolicy" → **Algieria** (Algier jako stolica, Atlas + Sahara na S).
- **C** "obecność jednego z największych jezior Afryki" → np. Uganda (Wiktoria) lub Tanzania (Tanganika), nie te 3 kraje.
- **D** "surowce mineralne (węgiel, złoto, diamenty, rudy)" → **RPA**.

</div>

<div class="krok">

**Wynik**:
- **Algieria** = **B**.
- **Nigeria** = **A**.
- **RPA** = **D**.

</div>

</div>

<div class="odpowiedz">
<div>

- **Algieria** = **B** (góry + Sahara na S od Algieru).
- **Nigeria** = **A** (delta Nigru + ropa).
- **Republika Południowej Afryki** = **D** (bogactwa mineralne).

</div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Typowy błąd</div>

  **Pomylenie Algerii z Nigerią.** Brzmienie podobne (Niger ≠ Nigeria ≠ Algieria), ale różne kontynentalne lokalizacje:
  - **Algieria** — N Afryki, Sahara, śródziemnomorski klimat.
  - **Niger** — Sahel, biedny kraj śródlądowy.
  - **Nigeria** — W Afryki nad Zatoką Gwinejską, najliczniejszy kraj Afryki (~220 mln).

  **Pomylenie surowców RPA z innymi krajami.** RPA = klasyk **diamentów i złota** (Kimberley, Witwatersrand). Botswana = diamenty. DR Konga = koltan, kobalt. Każdy kraj ma własną "wizytówkę" surowcową.

  **Pomyłka stolicy Algierii.** Stolica = **Algier** (na wybrzeżu Morza Śródziemnego). Na S od Algieru → Atlas Telski + Sahara.
</div>

## Punktacja CKE

- **2 pkt** — trzy poprawne dopasowania.
- **1 pkt** — dwa poprawne.

## Po co to umieć

**Czynniki rozmieszczenia ludności w Afryce**:
- **Pozytywne**: doliny rzek (Nil, Niger), wybrzeża morskie, jeziora (Wiktoria, Tanganika), góry z umiarkowanym klimatem (Etiopskie), surowce mineralne.
- **Negatywne**: pustynie (Sahara, Kalahari, Namib), dżungla (Kongo), klimat tropikalny ekstremalny, konflikty zbrojne, choroby tropikalne.

To **klasyczne ograniczenia geograficzne** rozwoju demograficznego Afryki, gdzie ludność koncentruje się **w pasach przyjaznych klimatycznie**.
