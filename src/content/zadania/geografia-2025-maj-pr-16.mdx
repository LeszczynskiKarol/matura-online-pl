---
arkusz: geografia-2025-maj-pr
subject: geografia
nr: 16
punkty: 2
typ: otwarte-krotkie
topic: gleby Polski, bielica, profil glebowy, burak cukrowy
difficulty: 4
tresc: |
  Fotografia profilu gleby z Polski (jasny poziom wymywania nad ciemniejszym wzbogacenia — typowa bielica). Wskaż A. mada, B. rędzina, C. brunatna, D. bielicowa. Uzasadnij małą przydatność dla buraka cukrowego.
odpowiedz: |
  **D. gleba bielicowa**

  **Mała przydatność dla buraka cukrowego wynika z:**
  - widoczna na fotografii **jasna warstwa wymywania (poziom albuwiku)** o jasnoszarym/białym kolorze — uboga w substancje organiczne i składniki mineralne (Ca, K, Mg);
  - gleba **kwaśna** (pH < 5) — burak cukrowy wymaga odczynu obojętnego/zasadowego;
  - powstaje na **piaskach** (sandry, wydmy) → mała pojemność wodna, słaba retencja;
  - obecność **poziomu orsztynu** (twarda warstwa wzbogacenia) ogranicza zasięg korzeni.
pulapka: |
  Burak cukrowy potrzebuje gleb żyznych, zasobnych w wapń, o odczynie pH 6,5-7,5 (czarnoziemy, mady, gleby brunatne). Bielice na piaskach są dla niego nieodpowiednie.
wymaganie: |
  VI.1 — rozpoznawanie typów gleb i ich przydatności rolniczej.
podobne:
  - geografia-2025-maj-pr-11
  - geografia-2025-maj-pr-25
---

import MaterialZrodlowy from "../../components/MaterialZrodlowy.astro";

## Treść zadania (CKE)

**Zadanie 16. (0–2)**

Na fotografii przedstawiono profil jednej z gleb występujących w Polsce.

<MaterialZrodlowy
  src="/arkusze/geografia/2025/arkusz-20.webp"
  alt="Profil glebowy: cienki poziom próchniczny, jasny poziom wymywania, ciemniejszy poziom wzbogacenia (orsztyn), piaszczysty"
  caption="Profil glebowy z Polski. Charakterystyczna jasna warstwa wymywania = typowa bielica."
  zrodlo="www.zasobynauki.pl"
  pdfUrl="https://cke.gov.pl/images/_EGZAMIN_MATURALNY_OD_2023/Arkusze_egzaminacyjne/2025/Geografia/MGEP-R0-100-2505-arkusz.pdf"
  pdfPage={20}
/>

**Zaznacz nazwę gleby, której profil przedstawiono na fotografii, a następnie wykaż, z czego wynika mała możliwość wykorzystania tej gleby pod uprawę buraka cukrowego. Odnieś się do odpowiedniej cechy widocznej na fotografii.**

- **A.** mada
- **B.** rędzina
- **C.** gleba brunatna
- **D.** gleba bielicowa ✓

Mała możliwość wykorzystania pod uprawę buraka cukrowego gleby przedstawionej na fotografii wynika z: ......................

## Co zrobić?

<div class="kroki-lista">

<div class="krok">

**Rozpoznawanie typów gleb po profilu**:
- **Mada** — jasna, mułowa, na rzecznym osadzie. Cienka próchnica.
- **Rędzina** — na wapieniach, ciemna, dużo wapnia.
- **Gleba brunatna** — jednolicie brunatna, gruba próchnica, na utworach gliniastych.
- **Bielica** — **charakterystyczna jasna warstwa wymywania** (eluwialna) między próchnicą a poziomem wzbogacenia. Powstaje pod lasami iglastymi na piaskach.

Z fotografii widać jasny pas pod próchnicą → **bielica = D**.

</div>

<div class="krok">

**Cechy bielicy ograniczające uprawę**:
- **Niska próchnica** — cienki poziom organiczny.
- **Jasna warstwa wymywania** — uboga w Ca, K, Mg, Fe (wszystko wymyte w dół).
- **Niskie pH** (4-5, kwaśna) — toksyczne dla większości upraw.
- **Piaski** w podłożu — mała retencja wody.
- **Poziom orsztynu** (wzbogacenia) — twarda warstwa, blokuje korzenie.

</div>

<div class="krok">

**Wymagania buraka cukrowego**:
- pH **obojętne/zasadowe** (6,5-7,5).
- Gleby **żyzne, głębokie**, dobrze odżywione.
- Idealne: **czarnoziemy** (Kujawy, Lubelszczyzna), **mady**, gleby brunatne.

Bielica → **całkowicie nieodpowiednia** (kwaśna, piaszczysta, mało próchnicy).

</div>

<div class="krok">

**Konstrukcja odpowiedzi (2 pkt)**:
1. Nazwa: **D — gleba bielicowa**.
2. Uzasadnienie z **odniesieniem do widocznej cechy** na fotografii (np. jasna warstwa wymywania, kwaśny odczyn, podłoże piaszczyste).

</div>

</div>

<div class="odpowiedz">
<div>

**D — gleba bielicowa**.

Uzasadnienie: na fotografii widoczna **jasna warstwa wymywania** (poziom eluwialny) → uboga w składniki mineralne; gleba **kwaśna**, powstała na **piaskach** → mała żyzność. Burak cukrowy wymaga gleb żyznych, obojętnych/zasadowych — bielica jest **nieprzydatna**.

</div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Typowy błąd</div>

  **Pomylenie bielicy z czarnoziemem.** **Czarnoziem** = bardzo ciemny, gruba próchnica, **brak** jasnego poziomu wymywania. **Bielica** = jasna warstwa pod próchnicą.

  **Brak odniesienia do fotografii.** CKE **wyraźnie** wymaga odniesienia do "cechy widocznej na fotografii". Sam fakt "bielica jest słaba" niewystarczający — wskaż **konkretne cechy z obrazu** (jasna warstwa, piaskowa).

  **Pomylenie wymagań upraw.** Pszenica, burak cukrowy, kukurydza — gleby żyzne, obojętne. Ziemniaki, żyto, tatarka — gleby uboższe (też bielice).
</div>

## Punktacja CKE

- **2 pkt** — poprawna nazwa + poprawne uzasadnienie z odniesieniem do cechy widocznej na fotografii.
- **1 pkt** — tylko nazwa albo tylko uzasadnienie.

## Po co to umieć

**Gleby Polski** — uproszczona klasyfikacja przydatności rolniczej:

| Typ | Pasują uprawy | Regiony |
|---|---|---|
| Czarnoziemy | pszenica, burak cukrowy, tytoń | Lubelszczyzna, Niżna Wisły |
| Mady | warzywa, zboża, sady | doliny rzek |
| Brunatne | zboża, ziemniaki | większość Polski |
| Bielice | żyto, ziemniaki | Pojezierza, sandry |
| Rędziny | sady, winorośl | Wyżyna Lubelska |
| Bagienne | łąki, pastwiska | Polesie, Biebrza |

Wybór upraw zależy od **pH, próchnicy, retencji wody**.
