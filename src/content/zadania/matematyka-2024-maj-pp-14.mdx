...
tresc: |
  ...
  Zbiór wszystkich rozwiązań nierówności $f(x) \geq 0$ jest przedziałem $\langle -2,\ 4 \rangle$.
  ...
  **A.** $f(-4) = f(6)$     **B.** $f(-4) = f(5)$     **C.** $f(-4) = f(4)$     **D.** $f(-4) = f(7)$
  ...
  ### Zadanie 14.4. (0-2)
  Na rysunkach A-F przedstawiono fragmenty wykresów funkcji $g$ oraz $h$ określonych wzorami $g(x) = f(x+3)$ oraz $h(x) = f(-x)$, gdzie $f$ jest funkcją z treści zadania. Każdej z funkcji $g, h$ przyporządkuj odpowiedni rysunek (A-F).
odpowiedz: |
...
<MaterialZrodlowy
  src="/arkusze/matematyka/2024-maj-pp/zad-14.webp"
