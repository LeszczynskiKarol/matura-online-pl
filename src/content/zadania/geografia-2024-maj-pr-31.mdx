---
arkusz: geografia-2024-maj-pr
subject: geografia
nr: 31
punkty: 2
typ: otwarte-krotkie
topic: OPEC, handel zagraniczny UE, ropa naftowa
difficulty: 4
tresc: |
  Mapa świata z państwami OPEC (czerwone) i 4 dodatkowymi krajami eksportującymi/importującymi ropę. 31.1 — ocena P/F. 31.2 — uzasadnij wpływ polityki OPEC na handel UE.
odpowiedz: |
  **31.1.** — **FP**
  1. **F** — państwa 2 (Wenezuela) i 3 (Rosja) NIE są tylko eksporterami; Rosja eksportuje a Wenezuela wstrzymywała eksport w XXI w. (kryzys polityczny).
  2. **P** — państwa 1 (Norwegia? USA? Kanada?) i 4 (Chiny? Indie?) mogą być w głównej mierze importerami ropy mimo własnego wydobycia.

  **31.2.** Polityka OPEC (regulacja wydobycia, ceny kwot OPEC) wpływa na handel UE:
  - **wzrost cen ropy** w OPEC → wzrost cen towarów eksportowanych przez UE (koszty produkcji);
  - może spowodować **ujemne saldo** krajów UE w obrotach z OPEC;
  - **zwiększenie importu** ropy z państw spoza OPEC (Norwegia, USA);
  - **wzrost importu komponentów** do produkcji samochodów elektrycznych (transformacja energetyczna).
wymaganie: |
  XIX.2 — związki między surowcami a strukturą handlu zagranicznego.
  XX.2 — problemy funkcjonowania UE.
podobne:
  - geografia-2024-maj-pr-27
  - geografia-2024-maj-pr-30
---

import MaterialZrodlowy from "../../components/MaterialZrodlowy.astro";

## Treść zadania (CKE)

**Zadanie 31.**

Na mapie czerwonym kolorem oznaczono państwa należące w 2020 roku do organizacji OPEC. Numerami 1–4 zaznaczono inne wybrane państwa.

<MaterialZrodlowy
  src="/arkusze/geografia/2024/arkusz-35.webp"
  alt="Mapa świata: państwa OPEC (czerwone) + 4 inne państwa (1–4) eksportujące lub importujące ropę"
  caption="Państwa OPEC w 2020 r. (czerwone) + 4 wybrane państwa oznaczone numerami 1–4."
  zrodlo="pl.wikipedia.org"
  pdfUrl="https://cke.gov.pl/images/_EGZAMIN_MATURALNY_OD_2023/Arkusze_egzaminacyjne/2024/geografia/MGEP-R0-100-2405-arkusz.pdf"
  pdfPage={35}
/>

### Zadanie 31.1. (0–1)

**Oceń, czy poniższe informacje są prawdziwe.**

| Nr | Zdanie | Ocena |
|---|---|---|
| 1. | Państwa oznaczone na mapie numerami 2 i 3 nie eksportowały ropy naftowej w końcu drugiej dekady XXI wieku. | P / **F** |
| 2. | Państwa oznaczone na mapie numerami 1 i 4 wydobywają ropę naftową, ale ze względu na duży popyt są importerami tego surowca. | **P** / F |

### Zadanie 31.2. (0–1)

**Uzasadnij, że polityka energetyczna państw zrzeszonych w organizacji OPEC wpływa na handel zagraniczny państw Unii Europejskiej.**

## Co zrobić?

### 31.1. — P/F o eksporterach/importerach ropy

<div class="kroki-lista">

<div class="krok">

**Zdanie 1**: "państwa 2 i 3 nie eksportowały ropy" → **FAŁSZ**. Patrząc na mapę, państwa 2 i 3 (np. Wenezuela, Rosja, Norwegia) **są** dużymi eksporterami ropy w XXI w.

</div>

<div class="krok">

**Zdanie 2**: "państwa 1 i 4 wydobywają, ale są importerami z powodu popytu" → **PRAWDA**. Klasyczny przykład: **USA** (do 2018 r.) i **Chiny** — produkują ropę, ale **popyt krajowy** przewyższa produkcję → muszą importować.

</div>

</div>

### 31.2. — wpływ OPEC na handel UE

<div class="kroki-lista">

<div class="krok">

**Mechanizm wpływu**:
1. **OPEC** (kraje arabskie + Wenezuela + Nigeria + inne) kontroluje **~30% światowej produkcji ropy** i ma silny wpływ na ceny.
2. **Polityka OPEC** = ograniczenie produkcji → **wzrost cen** ropy.
3. **UE importuje większość ropy** (z OPEC + Norwegii + Rosji) → wzrost kosztów importu.

</div>

<div class="krok">

**Skutki dla handlu UE**:
- **Wzrost kosztów produkcji** w UE (paliwa, plastiki, chemia).
- **Wzrost cen towarów eksportowanych** przez UE → mniejsza konkurencyjność.
- **Pogorszenie salda handlowego** UE z OPEC (więcej wydaje na ropę).
- **Wzrost importu ropy spoza OPEC** (Norwegia, USA — gaz LNG).
- **Przyspieszenie transformacji energetycznej** (samochody elektryczne, OZE) — by ograniczyć zależność.

</div>

</div>

<div class="odpowiedz">
<div>

**31.1. — FP**

**31.2.** Polityka OPEC → wzrost cen ropy → wzrost kosztów produkcji w UE → wzrost cen towarów eksportowanych, pogorszenie salda handlowego, wzrost importu spoza OPEC, transformacja energetyczna.

</div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Typowy błąd</div>

  **31.2: Brak związku z handlem UE.** Sama odpowiedź "ropa drożeje" niewystarczająca. Trzeba **konkretu** dla **handlu zagranicznego UE** (saldo, eksport, import, transformacja).

  **Pomylenie OPEC z OPEC+.** OPEC = 13 państw (Arabia Saudyjska, Iran, Irak, Wenezuela, Nigeria itd.). **OPEC+** = OPEC + Rosja, Kazachstan, Meksyk i inne — większa koalicja. CKE pyta o OPEC, więc trzymaj się tej organizacji.

  **Pomylenie krajów OPEC ze wszystkimi krajami naftowymi.** Norwegia, USA, Kanada, Rosja **nie są** członkami OPEC, choć są dużymi producentami.
</div>

## Punktacja CKE

- **31.1:** 1 pkt — dwie poprawne oceny.
- **31.2:** 1 pkt — poprawne uzasadnienie odnoszące się do wpływu polityki OPEC na handel UE.

## Po co to umieć

**OPEC** (Organization of Petroleum Exporting Countries):
- **Założenie**: 1960, główny cel — koordynacja polityki naftowej.
- **Najwięksi członkowie**: Arabia Saudyjska, Iran, Irak, ZEA, Kuwejt, Wenezuela, Nigeria, Libia.
- **Wpływ**: ok. 30-35% światowej produkcji + 80% rezerw.
- **Polityka**: regulacja **kwot produkcyjnych** → wpływ na ceny.

**UE** = **największy importer ropy na świecie**. Każdy wzrost cen OPEC = wzrost kosztów dla 450 mln Europejczyków. To powód **długoterminowej transformacji** UE w stronę OZE (Pakiet Fit for 55, REPowerEU).
