...
<div class="odpowiedz">
  <div>
    ...
    <ol>
      <li>Dziedzina: $\langle -4, 4 \rangle$</li>
      <li>Zbiór wartości: $\langle -3, 3 \rangle$</li>
      <li>$f(x) > 0$ dla $x \in \langle -4, 3 \rangle$</li>
      <li>$f(x) = 3$ dla $x \in \langle -2, 2 \rangle$</li>
    </ol>
  </div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Pułapka — dziedzina vs zbiór wartości</div>

To są **dwa zupełnie różne zbiory**:

- **Dziedzina** = wszystkie $x$, dla których $f$ jest określona (oś $OX$)
- **Zbiór wartości** = wszystkie $y$, jakie $f$ przyjmuje (oś $OY$)

Łatwo pomylić: można wybrać $\langle -4, 4 \rangle$ dla obu (oba są przedziały „od minus czegoś do plus czegoś"), ale **dziedzina** to $\langle -4, 4 \rangle$, a **zbiór wartości** to $\langle -3, 3 \rangle$.
</div>

<div class="pulapka">
  <div class="pulapka-label">Pułapka kolejności końców (kryterium dyskalkulii z CKE)</div>

Jeśli wpiszesz przedział w niewłaściwej kolejności (np. $\langle 4, -4 \rangle$ zamiast $\langle -4, 4 \rangle$) — CKE daje **1 pkt** za to konkretne zdanie (zamiast 0). To uwzględnienie trudności specyficznych dla osób z dyskalkulią.

Bezpieczna procedura: w przedziale **mniejsza liczba zawsze po lewej**. Sprawdź na osi liczbowej, że poruszasz się od mniejszej do większej.
</div>
