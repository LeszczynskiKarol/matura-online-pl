---
arkusz: geografia-2025-maj-pr
subject: geografia
nr: 2
punkty: 2
typ: otwarte-krotkie
topic: kartografia, skala, ortofotomapa, Kolei Goryczkowa
difficulty: 3
tresc: |
  Oblicz długość trasy Kolei Goryczkowej (pole J3/4) na ortofotomapie w skali 1:2 500, jeśli na mapie szczegółowej Tatr (1:55 000) trasa ma 2,9 cm.
odpowiedz: |
  **63,8 cm** (lub 638 mm)

  Obliczenia:
  - Długość w terenie: 2,9 cm × 55 000 = 159 500 cm = 1 595 m
  - Długość na ortofotomapie 1:2 500: 159 500 cm ÷ 2 500 × 1 = 159 500 / 2 500 = 63,8 cm

  Albo prościej: stosunek skali 55 000/2 500 = 22, więc długość na ortofotomapie = 2,9 × 22 = **63,8 cm**.
pulapka: |
  Większa skala (mianownik mniejszy) = większy obraz w terenie. 1:2 500 to większa skala niż 1:55 000 → trasa jest 22× dłuższa na ortofotomapie.
wymaganie: |
  I.7 — wykonywanie obliczeń geograficznych na podstawie skali.
podobne:
  - geografia-2025-maj-pr-1
  - geografia-2025-maj-pr-7
---

import MaterialZrodlowy from "../../components/MaterialZrodlowy.astro";

## Treść zadania (CKE)

**Zadanie 2. (0–2)**

Przyjmij, że długość trasy Kolei Goryczkowej (pole J3/4) na załączonej mapie szczegółowej Tatr wynosi **2,9 cm**.

**Oblicz długość trasy tej kolei na ortofotomapie wykonanej w skali 1 : 2 500. Zapisz obliczenia.**

<MaterialZrodlowy
  src="/arkusze/geografia/2025/mapa.webp"
  alt="Mapa szczegółowa Tatr — trasa Kolei Goryczkowej w polu J3/4"
  caption="Mapa szczegółowa Tatr (1:55 000). Kolej Goryczkowa — pole J3/4."
  zrodlo="CKE 2025"
  pdfUrl="https://cke.gov.pl/images/_EGZAMIN_MATURALNY_OD_2023/Arkusze_egzaminacyjne/2025/Geografia/MGEP-R0-100-2505-mapa.pdf"
/>

## Co zrobić?

<div class="kroki-lista">

<div class="krok">

**Wyznacz długość w terenie**. Mapa szczegółowa Tatr ma skalę **1:55 000** → 1 cm na mapie = 55 000 cm = 550 m w terenie.

$$L_{teren} = 2{,}9\,\text{cm} \cdot 550\,\text{m/cm} = 1595\,\text{m} = 159\,500\,\text{cm}$$

</div>

<div class="krok">

**Przelicz na ortofotomapę 1:2 500** (1 cm na mapie = 2 500 cm = 25 m w terenie):

$$L_{ortofotomapa} = \frac{L_{teren}}{2\,500} = \frac{159\,500\,\text{cm}}{2\,500} = 63{,}8\,\text{cm}$$

</div>

<div class="krok">

**Skrót — stosunek skal**:

$$\frac{55\,000}{2\,500} = 22$$

Czyli ortofotomapa pokazuje teren **22× większy** niż mapa szczegółowa.

$$L_{ortofotomapa} = 2{,}9\,\text{cm} \cdot 22 = 63{,}8\,\text{cm}$$

</div>

</div>

<div class="odpowiedz">
<div>

**Długość trasy na ortofotomapie: 63,8 cm**

</div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Typowy błąd</div>

  **Pomyłka kierunku skali**. Im **mniejsza** liczba w skali (np. 1:2 500), tym **większy** obraz terenu. Mapa szczegółowa 1:55 000 = mała skala, ogólny widok. Ortofotomapa 1:2 500 = duża skala, szczegóły.

  **Pomylenie cm z m**. Pamiętaj: w skali 1:55 000 → 1 cm = 55 000 cm = **550 m** (nie 55 m, nie 5,5 km).

  **Brak zapisu obliczeń.** Sam poprawny wynik bez sposobu obliczenia = -1 pkt.
</div>

## Punktacja CKE

- **2 pkt** — poprawne obliczenie z poprawnym wynikiem.
- **1 pkt** — poprawny sposób obliczenia z błędem rachunkowym **albo** poprawny wynik bez zapisu obliczeń.

## Po co to umieć

**Skala mapy** — kluczowy element:
- **Duża skala** (1:1 000 - 1:25 000): mapy szczegółowe, plany miast, ortofotomapy.
- **Średnia skala** (1:50 000 - 1:200 000): mapy topograficzne.
- **Mała skala** (1:1 000 000+): mapy przeglądowe, atlasy.

Operacja: **przeliczanie skali** to częste zadanie maturalne — pamiętaj wzór $L_{teren} = L_{mapa} \cdot M$ (gdzie M = mianownik skali w jednostkach mapy).
