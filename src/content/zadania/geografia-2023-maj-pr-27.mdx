---
arkusz: geografia-2023-maj-pr
subject: geografia
nr: 27
punkty: 1
typ: otwarte-krotkie
topic: ubóstwo edukacyjne Afryki, dostępność szkół, kultura
difficulty: 4
tresc: |
  Podaj jeden czynnik wraz z wyjaśnieniem, ograniczający dostęp do edukacji w państwach Afryki.
odpowiedz: |
  **Przykładowe czynniki (jeden z wyjaśnieniem):**
  - **Praca dzieci / tradycyjne zajęcia w gospodarstwie domowym** — ubóstwo rodzin wymusza pracę dzieci i uniemożliwia ich edukację.
  - **Kultura / tradycje** — wczesne zamążpójście dziewcząt (ich tradycyjna rola społeczna) jest przeszkodą dla edukacji.
  - **Brak stabilności politycznej / częste wojny** — konflikty sprzyjają pogłębieniu zjawiska ubóstwa.
  - **Brak infrastruktury edukacyjnej** — szkoły zbyt oddalone, niskiej jakości budynki, brak wyposażenia.
  - **Klimat** — występowanie pustyń utrudnia dojazd do szkół.
wymaganie: |
  XII.2 — zróżnicowanie dostępności do usług edukacyjnych na świecie.
podobne:
  - geografia-2023-maj-pr-17
  - geografia-2024-maj-pr-21
---


import MaterialZrodlowy from "../../components/MaterialZrodlowy.astro";

## Tresc zadania (CKE)

<MaterialZrodlowy
  src="/arkusze/geografia/2023/arkusz-38.webp"
  alt="Tresc zadania 27 z arkusza CKE 2023 PR (str. 38)"
  caption="Strona 38 arkusza CKE 2023 PR - zadanie 27."
  zrodlo="CKE 2023"
  pdfUrl="https://cke.gov.pl/images/_EGZAMIN_MATURALNY_OD_2023/Arkusze_egzaminacyjne/2023/geografia/MGEP-R0-100-2305-arkusz.pdf"
  pdfPage={38}
/>

## Co zrobić?

<div class="kroki-lista">

<div class="krok">

**Czynniki utrudniające dostęp do edukacji w Afryce** (kategorie):

**Ekonomiczne:**
- **Praca dzieci** — ubogie rodziny zmuszają dzieci do pracy (pole, dom, opieka nad rodzeństwem).
- Brak środków na szkolne opłaty, mundurki, materiały.

**Kulturowe / społeczne:**
- **Wczesne zamążpójście dziewcząt** (15-17 lat) → koniec edukacji.
- **Patriarchalne** wzorce — chłopcy do szkoły, dziewczynki do domu.
- **Bariera językowa** (klasa po francusku/angielsku, w domu lokalny język).

**Polityczne:**
- **Konflikty zbrojne** — zamknięte szkoły, dzieci-żołnierze, uchodźstwo.
- **Słabość państwa** — brak inwestycji w infrastrukturę edukacyjną.

**Geograficzne / infrastrukturalne:**
- **Odległość do szkoły** (np. wioska + szkoła kilkanaście km dalej, brak transportu).
- **Klimat** (pustynia, sezony deszczowe) utrudniający dotarcie.
- **Brak elektryczności** w szkołach.

</div>

<div class="krok">

**Konstrukcja odpowiedzi (1 pkt)**:
1. **Czynnik** — konkretny (np. "praca dzieci").
2. **Wyjaśnienie** — mechanizm (np. "ubogie rodziny zmuszają dzieci do pracy zarobkowej, co uniemożliwia chodzenie do szkoły").

**Sam czynnik = 0 pkt**. Trzeba **wyjaśnić mechanizm**.

</div>

</div>

<div class="odpowiedz">
<div>

**Klucz CKE — przykładowe czynniki + wyjaśnienia**:
- **Praca dzieci** → ubóstwo rodzin zmusza dzieci do pracy zamiast szkoły.
- **Tradycyjne wczesne zamążpójście dziewcząt** → koniec edukacji.
- **Konflikty zbrojne** → zamknięcie szkół, uchodźstwo, brak nauczycieli.
- **Brak infrastruktury** edukacyjnej (oddalone szkoły, niska jakość).
- **Klimat pustynny** → utrudniony dojazd do szkół.

</div>
</div>

<div class="pulapka">
  <div class="pulapka-label">Typowy błąd</div>

  **Czynnik bez wyjaśnienia** — odpowiedź "bieda" lub "wojna" daje **0 pkt**. CKE wymaga **mechanizmu**.

  **Niespecyficzność dla Afryki**: czynnik "lenistwo dzieci" — uniwersalny, nie specyficzny dla Afryki, więc słaby. Wybieraj czynniki **charakterystyczne** (praca dzieci, wczesne małżeństwa, konflikty zbrojne, pustynne klimaty).

  **Stereotypy**: "afrykanie są niezainteresowani edukacją" — błędny i obraźliwy. Wybierz **systemowe** czynniki, nie kulturowe stereotypy.
</div>

## Punktacja CKE

- **1 pkt** — poprawny czynnik **wraz z poprawnym wyjaśnieniem**.
- **0 pkt** — odpowiedź niespełniająca wymagań (np. sam czynnik bez wyjaśnienia).

## Po co to umieć

**Cele Zrównoważonego Rozwoju ONZ (SDG)** — cel 4 to **edukacja dla wszystkich**. Afryka subsaharyjska ma najwyższe wskaźniki dzieci poza szkołą (~17% globalnie). Główne bariery to **ubóstwo** (praca dzieci) i **kultura** (mała szansa na edukację dla dziewcząt). Konkretne interwencje: budowa szkół, programy żywieniowe, stypendia dla dziewcząt — działają, choć powoli.
